...
function Achivements() {
    ...
    const initEvent = async () => {
        setEvent(await getDocument('achievement'));
    };
    const saveEvent = async (dh: any) => {
        console.log();
        deletedTitles.forEach((d) => {
            deleteFile('achievement', d['Title']);
        });
        alert('saved');

        await setDocument(dh, 'achievement');
    };
    ...
    const add = async () => {
        ...
        console.log(await fileUpload(image, 'achievement', title + '_' + random));
        ...
        const js = {
            ...
            ImageUrl:
                'https://firebasestorage.googleapis.com/v0/b/priortizer.appspot.com/o/achievement%2F' +
                title +
                '_' +
                random +
                '?alt=media',
    ...
    const addEvent = () => {
        ...
        div.appendChild(createInput('Discription', () => {}, 'discription', 'text'));
